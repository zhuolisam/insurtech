...
function FilterButton() {
  ...
  return (
    <div className="relative inline-flex">
      <button ref={trigger} className="btn bg-white border-slate-200 hover:border-slate-300 text-slate-500 hover:text-slate-600" aria-haspopup="true" onClick={() => setDropdownOpen(!dropdownOpen)} aria-expanded={dropdownOpen}>
        ...
        <div ref={dropdown}>
            ...
            {option.map((item, index) => (
              <li className="py-1 px-3">
                <label className="flex items-center">
                  ...
                  <span className="text-sm font-medium ml-2">{option[index]}</span>
                </label>
              </li>
            ))}
          </ul>
          ...
          <div className="py-2 px-3 border-t border-slate-200 bg-slate-50">
            <ul className="flex items-center justify-between">
              ...
              <li>
                <button className="btn-xs bg-indigo-500 hover:bg-indigo-600 text-white" onClick={() => setDropdownOpen(false)} onBlur={() => setDropdownOpen(false)}>
                  Apply
                </button>
              </li>
            </ul>
